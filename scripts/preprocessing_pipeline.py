...
analytics and per-employee records, and writes 4 output files.
...
Outputs (all written to the same folder as --output):
    ...
    cleaned_hr_data.json     — per-employee records (JSON)
    cleaned_hr_data.csv      — per-employee records (CSV)
...
try:
    ...
    sys.exit(f"ERROR: Required config file not found: {_e.filename}\n"
             "Make sure columns.json, translations.json, and backend/refs/org_structure.json exist.")
...
def load_data(files: list[Path]) -> tuple[pd.DataFrame, dict]:
    ...
    # Log library versions to make cross-environment differences easy to diagnose
    try:
        import openpyxl
        log.info("  pandas %s | openpyxl %s", pd.__version__, openpyxl.__version__)
    except ImportError:
        log.info("  pandas %s | openpyxl not installed", pd.__version__)
...
def _read_file(path: Path) -> tuple[list[pd.DataFrame], list[dict], pd.DataFrame]:
    ...
    for sheet in hr_sheets:
        try:
            ...
            df  = _apply_template_column_mapping(raw, source=f"{path.name}::{sheet}")
...
def _apply_template_column_mapping(df: pd.DataFrame, source: str = "") -> pd.DataFrame:
    renamed = {k: v for k, v in TEMPLATE_RENAME.items() if k in df.columns}
    if not renamed:
        # No expected column names were found — the header row likely parsed differently
        # (common when openpyxl versions disagree on merged-cell propagation).
        log.warning(
            "  No template columns matched in %s. "
            "Check HEADER_ROW=%d and library versions. "
            "Actual columns: %s",
            source or "sheet", HEADER_ROW, list(df.columns)[:20],
        )
...
def _drop_empty_rows(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # If none of the key columns exist the mapping step failed entirely; keep
    # all rows so the caller's warning (from _apply_template_column_mapping)
    # is the actionable signal rather than silently dropping everything.
    if not key_cols:
...
def parse_employment_status(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def parse(raw) -> str:
        ...
        if val in ("", "nan"):
            return "Active"
        mapped = STATUS_MAP.get(val) or STATUS_MAP.get(val.lower())
        if mapped:
            return mapped
        if val not in _seen_unknown:
            _seen_unknown.add(val)
            log.warning("  Unknown status value (defaulting to Active): %r", val)
        return "Active"
...
# Stage 3 — Enrich (alignment, used internally for institute analytics only)
...
def build_hr_metrics(df: pd.DataFrame) -> tuple[list[dict], dict]:
    """Build institute analytics and org-wide summary. df must already have alignment columns."""
    log.info("Stage 3 — Enrich")

    if len(df) == 0:
        raise ValueError("No staff found after filtering.")

    institutes = build_institute_documents(df, full_df=df)
    ...
    summary = {
        ...
        "admin_exec_pct": percentage(n_admin + n_exec,  total),
        "n_executive":    n_exec,
...
def _weighted_alignment(f1, f2, f3, p1, p2, p3, match_fn) -> str:
    # Filter out empty / nan-string areas before any calculation
    entries = [(f, p) for f, p in ((f1, p1), (f2, p2), (f3, p3)) if f and f != "nan"]
    if not entries:
        return "Unclassified"

    total_weight = sum(p for _, p in entries)
    if total_weight == 0:
        return "Unclassified"

    matched_weight = sum(p for f, p in entries if match_fn(f))
...
def percentage(n: float, total: float) -> float:
...
def _normalise_focus_pcts(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _fix_row(row):
        # Zero out pcts for empty areas first so they don't distort the total
        for p, a in zip(pct_cols, area_cols):
            if not row[a] or row[a] in ("nan", ""):
                row[p] = 0.0
...
# Employee columns written to cleaned_hr_data.json and .csv
# Alignment variables are intentionally excluded — they live only in institution_hr.json
_STAFF_COLS = [
...
def _save_output(df: pd.DataFrame, meta: dict, out_path: Path):
    out_dir = out_path.parent
    out_dir.mkdir(parents=True, exist_ok=True)
    meta_json = {k: v for k, v in meta.items() if k != "dropped_rows"}

    # Compute alignment internally — used only for institute analytics
    df_aligned = classify_alignment(df.copy())
    institutes, summary = build_hr_metrics(df_aligned)

    # File 1: institution_hr.json — summary + institute analytics
    institution_path = out_dir / "institution_hr.json"
    institution_out  = {
    ...
    institution_path.write_text(json.dumps(institution_out, ensure_ascii=False, indent=2), encoding="utf-8")
    log.info("Written to %s  (%d institutes)", institution_path, len(institutes))

    # File 2: cleaned_hr_data.json — per-employee records (no alignment columns)
    ...
    out_path.write_text(json.dumps(staff_records, ensure_ascii=False, indent=2), encoding="utf-8")
    log.info("Written to %s  (%d employees)", out_path, len(staff_records))

    # File 3: cleaned_hr_data.csv — per-employee records (no alignment columns)
    csv_path = out_path.with_suffix(".csv")
    df[staff_cols].to_csv(csv_path, index=False, encoding="utf-8-sig")
    log.info("Written to %s  (%d employees)", csv_path, len(df))

    # File 4: dropped_rows_report.txt — written by _report_dropped_rows (called in main)

    if meta.get("errors"):
        log.warning("%d loading error(s) — see 'meta.errors' in institution_hr.json", len(meta["errors"]))
...
def main():
    ...
    _report_dropped_rows(meta, args.output.parent)
    ...
    for file in [f for f in files if not f.name.startswith("~$")]:
        ...
        try:
            ...
            if args.report_focus_areas:
                _report_focus_areas(df, suffix="")
                continue
            out_path = out_dir / (file.stem + "_cleaned_hr_data.json")
...
def _parse_args():
    ...
    parser.add_argument("--per-file",           action="store_true",
                        help="Process each file separately and save one set of outputs per file")
